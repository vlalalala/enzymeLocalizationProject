    ...
    for file_idx, file in enumerate(files):
        ...
        for species in reaction_network.species:
            if file_idx == 0:
                ...
                label = None
            ax[3].plot(x, df[f"{species.name}_absolute"], label=label, color=colors[species.name])
            ax[4].plot(x, df[f"{species.name}_relative"], label=label, color=colors[species.name])
