...
def get_data(folder):
    ...
    for folder_name in os.listdir(folder):
        ...
        if match:
            ...
            data[index] = (flux, michaelis_menten_constant, concentration)
    return data

def plot_data(folder):
    fig, ax = plt.subplots(1, 2, figsize = (4,3), gridspec_kw={'width_ratios': [1, 0.1]})
    data = get_data(folder)
    df = pd.DataFrame(data.values(), columns=['flux', 'michaelis_menten_constant', 'concentration'])
    scatterplot = ax[0].scatter(df["michaelis_menten_constant"], df["flux"], c = df['concentration'])
    ax[0].set_xlabel("michaelis_menten_constant")
    ax[0].set_ylabel("flux")
    fig.colorbar(scatterplot, cax=ax[1], label="average X concentration \n in volume with enzyme")
    #ax.set_ylabel()
...
if __name__ == "__main__":
    ...
    # python data/02e_enzymaticXtoY_2InnerBoundaries_modifyingKm/analysis.py data/02e_enzymaticXtoY_2InnerBoundaries_modifyingKm