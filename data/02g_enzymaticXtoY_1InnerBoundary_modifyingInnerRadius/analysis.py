    ...
    for folder_name in os.listdir(folder):
        ...
        if match:
            ...
            if os.path.isfile(fluxes_file):
                ...
                flux = None
            data[index] = (inner_radius, flux, index)
    return data
...
def plot_data(folder):
    ...
    df = pd.DataFrame(data.values(), columns=['inner_radius', 'flux', 'index'])
    
    ax.scatter(df["inner_radius"], df["flux"])
    ax.set_xlabel("relative membrane radius")
    ax.set_ylabel("flux")
...
if __name__ == "__main__":
    ...
    # python data/02g_enzymaticXtoY_1InnerBoundary_modifyingInnerRadius/analysis.py data/02g_enzymaticXtoY_1InnerBoundary_modifyingInnerRadius