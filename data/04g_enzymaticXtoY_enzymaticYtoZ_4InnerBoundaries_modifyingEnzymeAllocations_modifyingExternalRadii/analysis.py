...
def get_data(folder):
    ...
    for folder_name in os.listdir(folder):
        ...
        if match:
            ...
            x_axis_value = [region for region, val in allocationA.items() if val == 1][0]         
            y_axis_value = [region for region, val in allocationB.items() if val == 1][0]  
            fluxes_file = os.path.join(combined_folder, "fluxes.json")
            if os.path.isfile(fluxes_file):
                z_axis_value = load_json(fluxes_file)["Z"]
            else:
                z_axis_value = None
            data[index] = (x_axis_value, y_axis_value, z_axis_value, external_radius, index)
...
def plot_data(folder):
    ...
    df = pd.DataFrame(data.values(), columns=['x', 'y', 'flux', 'external_radius', 'index'])
    ...
    for row, external_radius in enumerate(external_radii):
        ...
        ax[row][0].scatter(current_df['x'], current_df['y'], color='red', s=5, zorder=5)
        fig.colorbar(mesh, cax=ax[row][1], label='flux')
        for _, point in current_df.iterrows():
            ax[row][0].annotate(str(point['index']).lstrip('0'), (point['x'], point['y']))

        # Set log scale on whichever axes need it
        ax[row][0].set_xlabel("region in which enzyme A is")
        ax[row][0].set_ylabel("region in which enzyme B is")
...
if __name__ == "__main__":
    ...
    # python data/04g_enzymaticXtoY_enzymaticYtoZ_4InnerBoundaries_modifyingEnzymeAllocations_modifyingExternalRadii/analysis.py data/04g_enzymaticXtoY_enzymaticYtoZ_4InnerBoundaries_modifyingEnzymeAllocations_modifyingExternalRadii