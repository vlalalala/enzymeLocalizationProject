...
def get_data(folder):
    ...
    for folder_name in os.listdir(folder):
        ...
        if match:
            ...
            x_axis_value = geometry["geometry_config"]["internal_membrane_relative_radii"][1]
            ...
            if os.path.isfile(fluxes_file):
                y_axis_value = load_json(fluxes_file)["Z"]/X_external_concentration
            else:
                y_axis_value = None

            average_Y_concentration = calculate_average_concentration_of_Y_in_inner_compartment_with_enzyme_B(combined_folder)
            data[index] = (x_axis_value, y_axis_value, X_external_concentration, average_Y_concentration)
    return data
...
def plot_data(folder):
    ...
    df = pd.DataFrame(data.values(), columns=['x', 'flux', 'external_concentration', 'average_Y_concentration'])
    for k_val, group in df.groupby('external_concentration'):
        ax[0].scatter(
            group["x"],
            group["flux"],
            label=str(k_val),
    ...
    ax[0].legend(title="external X concentration")
    ax[0].set_xlabel("relative radius of most inner membrane r*/R")
    ax[0].set_ylabel("flux / external concentration X")
    ax[1].set_xlabel("average Y concentration where B enzyme located")
...
if __name__ == "__main__":
    ...
    # python data/04_enzymaticXtoY_enzymaticYtoZ_3InnerBoundaries_modifyingPositionOf1EnzymeRegion_AInside/analysis.py data/04_enzymaticXtoY_enzymaticYtoZ_3InnerBoundaries_modifyingPositionOf1EnzymeRegion_AInside