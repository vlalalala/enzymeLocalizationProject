...
def get_data(folder):
    ...
    for folder_name in os.listdir(folder):
        ...
        if match:
            ...
            michaelis_menten = enzymatic_reactions_df.loc[
                (enzymatic_reactions_df["enzyme"] == "A"),
                "k_M"].item()
            data[index] = (inner_radius, outer_inner_radius, flux, michaelis_menten)
    return data

def plot_data(folder):
    data = get_data(folder)
    df = pd.DataFrame(data.values(), columns=['inner_radius', 'outer_inner_radius', 'flux', "michaelis_menten"])
    # only one value of Km here
    fig, ax = plt.subplots(1, 1, figsize = (4,3))
    michaelis_menten = np.sort(df["michaelis_menten"].unique())
    for michaelis_menten_idx, michaelis_menten_value in enumerate(michaelis_menten):
        #if michaelis_menten_value>1e-6 or michaelis_menten_value<1e-8:
        #    continue
        current_df = df[df["michaelis_menten"]==michaelis_menten_value]
        current_df = current_df.sort_values("inner_radius")
        ax.plot(current_df["inner_radius"], current_df["flux"],
                label = "{:.2E}".format(michaelis_menten_value), alpha = 1)
    
    ax.set_xlabel("inner radius")
    ax.set_ylabel("flux")
    ax.legend(title="MM constant",loc='center left', bbox_to_anchor=(1, 0.5))
...
def plot_steady_states(folder):
    ...
    fig, ax = plt.subplots(len(michaelis_menten), len(inner_radii), figsize = (4*len(inner_radii), 3*len(michaelis_menten)))
    for michaelis_menten_idx, michaelis_menten_value in enumerate(michaelis_menten):
        
        for inner_radius_idx, inner_radius in enumerate(inner_radii):
            current_df = df[(df["michaelis_menten"]==michaelis_menten_value)&(df["inner_radius"]==inner_radius)]
            ...
            combinations = filter_combined_folders(
                ...
                criteria_csv=
                    {
                        "options_enzymatic_reactions": {"enzyme": {"A": {"k_M": f"{michaelis_menten_value}"}}}
                    }
            )
            ...
            ax[michaelis_menten_idx][inner_radius_idx].imshow(img)
            ax[michaelis_menten_idx][inner_radius_idx].axis("off")
            ax[michaelis_menten_idx][inner_radius_idx].set_title(f"MM constant = {michaelis_menten_value}, \n inner radius = {inner_radius}")
...
if __name__ == "__main__":
    ...
    # python data/05i_spontaneousXtoY_enzymaticYtoZ_2InnerBoundaries_modifyingPositionOfEnzymeRegion_modifyingKm_lowPermeability/analysis.py data/05i_spontaneousXtoY_enzymaticYtoZ_2InnerBoundaries_modifyingPositionOfEnzymeRegion_modifyingKm_lowPermeability